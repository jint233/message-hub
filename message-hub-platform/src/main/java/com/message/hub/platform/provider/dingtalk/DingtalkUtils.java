...
public class DingtalkUtils {
    ...
    public static String getToken(String appKey, String appSecret) throws ApiException {
        // 尝试从缓存中获取令牌
        String token = CaffeineCache.getToken("dingTalk-" + appKey);
        if (token == null) {
            ...
            // 将获取到的令牌及其过期时间存入缓存
            token = CaffeineCache.setToken("dingTalk-" + appKey, oapiGettokenResponse.getExpiresIn(), oapiGettokenResponse.getAccessToken());
        }
        // 返回令牌
        return token;
    }
     ...
     */
    public static DingTalkClient getBotClient(String accessToken, String secret) throws NoSuchAlgorithmException, InvalidKeyException {
        // 获取当前时间戳，用于签名的时效性验证
     ...
     */
    public static DingTalkClient getChatClient() {
